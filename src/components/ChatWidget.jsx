...
export const ChatWidget = () => {
  ...
  return (
    <div className="fixed bottom-[72px] md:bottom-6 right-4 md:right-6 z-[90] flex flex-col items-end">
      <AnimatePresence>
        {isOpen && (
          <motion.div
      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.div
            ...
            className="relative mb-3 mr-1"
          >
            <button
              onClick={() => setIsOpen(true)}
              className="bg-zinc-900 hover:bg-zinc-800 text-white text-xs font-display tracking-wider font-bold px-4 py-2 rounded-full border border-zinc-700 shadow-xl shadow-black/50 transition-colors"
            >
              Ask AI
            </button>
            {/* Speech bubble tail */}
            <div className="absolute -bottom-1.5 right-6 w-3 h-3 bg-zinc-900 border-b border-r border-zinc-700 rotate-45 z-[-1]" />
          </motion.div>
        )}
      </AnimatePresence>
