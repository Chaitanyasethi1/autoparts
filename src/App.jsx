...
const Hero = () => {
  return (
    <section className="relative min-h-[95svh] md:min-h-screen flex items-center justify-center overflow-hidden pt-16 sm:pt-20 md:pt-0 pb-6 sm:pb-20 md:pb-0">
      <video
        src="/hero-video.mp4"
        autoPlay
        loop
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-8 text-left w-full md:mt-16">
        ...
        >
          ...
          {/* Lower Block: Pushed 1cm down on mobile with 3-line paragraph text */}
          <div className="mt-[24vh] sm:mt-0 flex flex-col gap-3 sm:gap-4">
            <p className="font-body text-zinc-200 text-[13px] sm:text-base md:text-xl mb-1 sm:mb-4 font-bold leading-snug sm:leading-relaxed max-w-xl" style={{ textShadow: '0 2px 10px rgba(0,0,0,0.9), 0 0 20px rgba(0,0,0,0.8)' }}>
              From vehicle inspections & engine diagnostics to new tires and brake repair. Our certified mechanics deliver fast, reliable service in Stoney Creek & nearby areas.
            </p>
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.button
          ...
          exit={{ opacity: 0, y: 20 }}
          onClick={scrollToTop}
          className="fixed bottom-[72px] left-4 md:bottom-6 md:left-6 z-[60] bg-zinc-900 border border-white/10 text-white w-10 h-10 rounded-full shadow-xl flex items-center justify-center hover:bg-zinc-800 transition-colors"
